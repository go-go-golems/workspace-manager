...
func (c *CliGitClient) ListBranches(ctx context.Context, repo RepositoryHandle) ([]string, error) {
    out, err := runGit(ctx, repo.Path(), "branch", "-a")
    ...
    for _, l := range lines {
        ...
        if l == "" || strings.Contains(l, "HEAD ->") { continue }
...
func (c *CliGitClient) ListTags(ctx context.Context, repo RepositoryHandle) ([]string, error) {
    ...
    if err != nil { return nil, err }
    if len(out) == 0 { return []string{}, nil }
...
func (c *CliGitClient) Status(ctx context.Context, repo RepositoryHandle) (Status, error) {
    ...
    if len(out) == 0 { return st, nil }
    lines := strings.Split(strings.TrimSpace(string(out)), "\n")
    for _, l := range lines {
        if l == "" || len(l) < 3 { continue }
        idx := l[0]
        wt := l[1]
        path := strings.TrimSpace(l[2:])
        if idx != ' ' && idx != '?' { st.StagedFiles = append(st.StagedFiles, path) }
        if wt != ' ' && wt != '?' { st.ModifiedFiles = append(st.ModifiedFiles, path) }
        if idx == '?' && wt == '?' { st.UntrackedFiles = append(st.UntrackedFiles, path) }
    }
    return st, nil
}
...
func (c *CliGitClient) AheadBehind(ctx context.Context, repo RepositoryHandle, upstream string) (int, int, error) {
    if upstream == "" {
        ...
        if len(fields) != 2 { return 0, 0, errors.Errorf("unexpected rev-list output: %s", string(out)) }
...
func (c *CliGitClient) CreateBranch(ctx context.Context, repo RepositoryHandle, name string, track bool, baseRef string) error {
    ...
    if track { args = append(args, "--track") }
    if baseRef != "" { args = append(args, baseRef) }
